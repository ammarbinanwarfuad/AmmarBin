...
export function BlogClient({ initialBlogs }: { initialBlogs: Blog[] }) {
  const { data, isLoading, mutate: mutateSWR } = useSWR('/api/blog?includeUnpublished=true', fetcher, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/api/blog/${editingBlog}` : "/api/blog";
      ...
      if (response.ok) {
        ...
        toast.error(isEditing ? "Failed to update blog post" : "Failed to add blog post");
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            ...
            <Button onClick={syncBlogs} variant="outline" className="gap-2 text-sm">
                    ...
                    <div className="flex items-start gap-2 sm:gap-4 flex-1 min-w-0 w-full sm:w-auto">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <CardTitle className="text-base sm:text-lg break-words">{blog.title}</CardTitle>
                          {(() => {
                            const isScheduled = blog.publishedDate && new Date(blog.publishedDate) > new Date() && !blog.published;
                            if (isScheduled) {
                              return (
                                <span className="text-xs bg-blue-500/20 text-blue-500 px-2 py-1 rounded flex-shrink-0">
                                  Scheduled
                                </span>
                              );
                            }
                            return blog.published ? (
                              <span className="text-xs bg-green-500/20 text-green-500 px-2 py-1 rounded flex-shrink-0">
                                Published
                              </span>
                            ) : (
                              <span className="text-xs bg-yellow-500/20 text-yellow-500 px-2 py-1 rounded flex-shrink-0">
                                Draft
                              </span>
                            );
                          })()}
                        ...
                        <CardDescription className="mt-1 text-xs sm:text-sm break-words">
                          ...
                          {(() => {
                            const publishedDate = (blog as Blog & { publishedDate?: string }).publishedDate;
                            if (publishedDate) {
                              const isScheduledForFuture = new Date(publishedDate) > new Date() && !blog.published;
                              if (isScheduledForFuture) {
                                return <> • Scheduled: {format(new Date(publishedDate), "MMM dd, yyyy 'at' HH:mm")}</>;
                              } else if (blog.published) {
                                return <> • Published: {format(new Date(publishedDate), "MMM dd, yyyy 'at' HH:mm")}</>;
                              }
                            }
                            return null;
                          })()}
                        </CardDescription>
                        <p className="text-xs sm:text-sm text-muted-foreground mt-2 break-words line-clamp-2">
                          {blog.excerpt}
                        </p>
