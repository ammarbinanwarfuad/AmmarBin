...
export default function AdminDashboard() {
  ...
  const { data: recent } = useSWR('/api/admin/recent', fetcher);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {analytics && [
              { label: 'Projects', value: analytics.totals.projects },
              { label: 'Blogs', value: analytics.totals.blogs },
              { label: 'Messages', value: analytics.totals.messages },
              ...
              <Card key={m.label}>
                ...
                <CardContent>
                  ...
                  <div className="text-xs text-muted-foreground mt-1">7d: {analytics.last7d[m.label.toLowerCase()]}, 30d: {analytics.last30d[m.label.toLowerCase()]}</div>
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                <ul className="space-y-2">
                  {recent?.messages?.map((m: { _id: string; name: string; subject?: string }) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between text-sm mb-3">
                  <span>{seo ? `${seo.issues.length} issues across ${seo.total} posts` : 'Loading...'}</span>
                  {seo && (
                    <button
                ...
                {seo?.issues && showSeoDetails && (
                  <ul className="space-y-2 text-sm">
                    {seo.issues.map((i: { slug: string; title: string; missingMeta?: boolean; missingImage?: boolean; shortContent?: boolean }) => (
          ...
          <div className="mt-8">
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between text-sm mb-3">
                  <span>{links ? `${links.broken.length} projects with issues` : 'Running...'}</span>
                  {links && (
                    <button
                      className="text-primary hover:underline"
                ...
                {links?.broken && showLinkDetails && (
                  <ul className="space-y-2 text-sm">
                    {links.broken.map((row: { title: string; live: { ok: boolean; url?: string; status?: number }; github: { ok: boolean; url?: string; status?: number } }, idx: number) => (
          ...
          <div className="mt-8">
            <Card>
              ...
              <CardContent>
                <div className="text-sm">DB latency: {system?.dbLatencyMs} ms</div>
                <div className="text-xs text-muted-foreground mt-1">ENV: NEXTAUTH_URL {system?.env?.NEXTAUTH_URL ? '✓' : '✗'}, Cloudinary {system?.env?.CLOUDINARY ? '✓' : '✗'}, GitHub {system?.env?.GITHUB_USERNAME ? '✓' : '✗'}</div>
              </CardContent>
            </Card>
          </div>
                ...
                {webVitals?.stats ? (
                  <div className="space-y-4">
                    {['LCP', 'INP', 'CLS', 'FCP', 'TTFB'].map((vital) => {
                      const stats = webVitals.stats[vital];
                    ...
                    <div className="text-xs text-muted-foreground mt-4 pt-4 border-t border-border">
                      Total metrics collected: {webVitals.total || 0}
                    </div>
                  </div>
                ) : (
