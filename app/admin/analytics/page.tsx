    ...
    <div className="min-h-screen bg-background p-3 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          <div>
            <h1 className="text-2xl md:text-4xl font-bold">Visitor Analytics</h1>
            ...
            {stats.topPages?.length > 0 ? (
              <div className="space-y-4">
                {stats.topPages.map((page: { path: string; views: number }, index: number) => (
                  <div key={page.path} className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
                    <div className="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                      <span className="text-muted-foreground text-sm md:text-base flex-shrink-0">#{index + 1}</span>
                      <span className="font-medium text-sm md:text-base truncate">{page.path}</span>
                    </div>
                    <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
                      <div className="text-right">
                        <div className="font-semibold text-sm md:text-base">{page.views}</div>
                        <div className="text-xs text-muted-foreground">views</div>
                      </div>
                      <div className="w-24 md:w-32 bg-secondary rounded-full h-2 relative">
                        <div
