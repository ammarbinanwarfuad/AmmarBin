...
export function MediaClient({ initialMedia }: { initialMedia: MediaData }) {
  ...
  const { data, isLoading, mutate } = useSWR(
    `/api/admin/media?search=${debouncedSearchTerm}&type=${resourceType}`,
    fetcher,
    {
  ...
  const media = data?.resources || [];

  const handleDelete = async (publicId: string) => {
    setDeletingId(publicId);
    try {
      // Get the actual resource type from the resource object
      const resource = (media as MediaResource[]).find(r => r.public_id === publicId);
  ...
  const toggleSelectAll = () => {
    if (selectedItems.size === media.length) {
      ...
      setSelectedItems(new Set(media.map((r: MediaResource) => r.public_id)));
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      const deletePromises = Array.from(selectedItems).map(async (publicId) => {
        try {
          // Get the actual resource type from the resource object
          const resource = media.find((r: MediaResource) => r.public_id === publicId);
          const actualResourceType = resource?.resource_type || resourceType || "image";
          const response = await fetch(`/api/admin/media?publicId=${publicId}&resourceType=${actualResourceType}`, {
            method: "DELETE",
          });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div>
          <div className="mb-4 text-sm text-muted-foreground">
            {media.length} {resourceType === "all" ? "items" : resourceType + "s"} found
          </div>
          {(
            <>
              {media.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {(media as MediaResource[]).map((resource) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-[10px] sm:text-xs text-muted-foreground mb-2 sm:mb-3 truncate">
                        ...
                        <div className="flex gap-1 sm:gap-2">
                          <Button
                            ...
                            className="flex-1 h-7 sm:h-8 px-1.5 sm:px-2 text-[10px] sm:text-xs"
                          >
