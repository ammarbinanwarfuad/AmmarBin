      ...
      if (!response.ok) {
        ...
        if (token && (response.status === 401 || response.status === 403)) {
          const errorMsg = response.status === 401 
            ...
            : 'GitHub API permission denied. For fine-grained tokens, grant: Repository permissions > Metadata (Read) and Contents (Read). For classic tokens, grant: repo scope.';
...
export async function getRepoTopics(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // If 401/403, log detailed permission error but don't throw - topics are optional
      if (response.status === 401 || response.status === 403) {
        console.warn(
          `⚠️ Cannot fetch topics for ${owner}/${repo} (${response.status}). ` +
          `For fine-grained tokens: Grant Repository permissions > Metadata (Read) and Contents (Read). ` +
          `For classic tokens: Grant 'repo' scope.`
        );
      } else {
        console.error(
